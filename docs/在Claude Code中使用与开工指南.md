...
- **host = codex**：Claude Code 没有 Cola 的原生 `gen_video` 工具，所以视频生成走 `listenhub` CLI 路径（模型 `doubao-seedance-2-pro`）。agent 会自动选这条路，你不用干预。
...
```bash
bash ~/.claude/skills/misheard-lyrics-video/scripts/check-deps.sh --host codex
```

期望：`SUMMARY: 0 error(s)`。`WARN:` 可容忍（见下文「配套 skill 的 WARN」），`ERROR:` 必须修复。
...
存在即已装。若仓库内容更新过，重跑 rsync 同步（排除规则与 `install.sh` 一致）：
...
> **配套 skill 的 WARN（可忽略）**：`check-deps.sh --host codex` 会报 4 条 `WARN: Companion Skill 'xxx' is not installed under ~/.codex/skills`。这是脚本只认 cola/codex 两个宿主、对 Claude Code 的盲区。配套 skill 已装在 `~/.claude/skills/`（Claude Code 实际加载位置），自检误报，不影响开工。
